...
function BrandsWorkedWith({ namespace = "translation" }) {
  ...
  return (
    <div
      className="h-[100dvh] w-full sm:min-h-[75vh] sm:pt-10 sm:h-auto"
      ref={carouselRef}
    >
      <Swiper
        ...
        {brands.map((brand, index) => (
          <SwiperSlide key={index}>
            ...
            >
              ...
              <div
                className={`w-full bg-[#00050f] p-5 flex flex-col text-left transition-all duration-500 ease-in-out overflow-hidden ${isHovered === index ? "h-0 opacity-0 p-0" : " opacity-100"}`}
              >
                <h3
                  className="font-bold text-xl uppercase mb-1 text-white leading-tight"
                  style={{ fontSize: "clamp(12px, min(6.5vw, 2.2vh), 16px)" }}
                >
                  {brand.name}
                </h3>
                <p className="text-gray-400 text-[10px] font-medium tracking-wider uppercase mb-3">
                  {brand.category}
                </p>
