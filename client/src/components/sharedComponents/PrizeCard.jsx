          ...
          <div className='max-w-sm w-full rounded-2xl shadow-xl bg-white p-4 md:p-6 border border-gray-100 mx-auto'>
            ...
            <div className='flex flex-col items-start w-full mb-3'>
              <div className='flex items-baseline gap-1'>
                <h2 className='font-extrabold text-gray-900 tracking-tight' style={{ fontSize: "clamp(22px, min(4vw, 5vh), 40px)" }}>
                  {formatPrice(currentPrice)}<span className='text-gray-500 font-medium' style={{ fontSize: "clamp(10px, min(1.5vw, 2vh), 14px)" }}>{t('hero_price_suffix')}</span>
                </h2>
              </div>
              <span className='text-gray-500 font-medium my-2' style={{ fontSize: "clamp(11px, min(1.5vw, 1.8vh), 13px)" }}> {t('hero_price_comparison')}</span>
              
              <p className='text-gray-600 leading-snug font-medium' style={{ fontSize: "clamp(12px, min(1.5vw, 1.8vh), 16px)" }}>
            ...
            <div className='space-y-3 mb-4'>
              <ul className='space-y-2'>
                {[1, 2, 3, 4, 5].map((num) => (
                  <li key={num} className='flex items-start gap-2 text-gray-700' style={{ fontSize: "clamp(12px, min(1.5vw, 1.8vh), 15px)" }}>
